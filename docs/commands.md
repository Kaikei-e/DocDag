...
`validate` builds the graph, runs the structural checks and the configured rules over the whole of
it, and exits 1 if any finding is an error. The four format sections below are the shapes that
report takes, and the fifth is the findings it deliberately leaves out;
[checks.md](checks.md) is what each finding means.
...
(`--touching` narrows the shipped vault's nine findings to the one clause this is about):

```console
$ cd testdata/fixtures/spec-vault
$ docdag validate --show-suppressed --touching spec/clauses/UZ-V-006.md
spec/clauses/UZ-V-006.md:4: ERROR modality_conflict UZ-V-006: is MAY and UZ-V-008 is SHOULD_NOT about topic/inferential-grader, suppressed by excepts UZ-V-006 -> UZ-V-008 (scope: only where the run also records a calibration measure)
as of 2026-09-02
(8 findings hidden)
```
...
The third layer reads fixtures a rule is written with rather than generated by hand:
[`new --fixture`](#new---fixture) writes the `ruleid/` and `ok/` pair from the rule's own condition.
...
`docdag stats` reports the corpus by degree — how many documents there are, how many bind, what
fraction is orphaned, one row per edge type, the distribution of supersedes chain depths and the ten
documents most referred to:
...
chain depth 0       3
chain depth 1       1
references to 0001  1
```

Chain depth is measured over `supersedes`, so a configuration declaring no edge of that name has no
lineage to describe and exits 3; `--fields`, below, answers for such a corpus anyway.
...
its row counts the whole corpus rather than the documents that write the key. The field report needs
no edge type, so it answers for a corpus the degree statistics cannot describe. `--format json`
writes `{"fields": [...]}` with the same values, `deprecated` always present and `last_change` left
...
The other thing `new` writes is a corpus rather than a document: the pair
[`lint --fixtures`](#lint) reads as its third layer.
...
Three JSON reports are headed by a version — `validate --format json`, `context --format json` and
`lint --format json` — a `schema_version` and, where the configuration names one, a
`preset_version`. The first two are at `schema_version: 2`; the lint report is a different kind of
report at `schema_version: 1` and says so in a `kind` field of its own. `stats --format json` is an
object as well but carries no version, being a report of counts rather than of findings. The
listings are arrays of records rather than objects (`query`, `query --fields`, `resolve`), so they
carry no header either; `preset_version` is read from `validate` beside them. The text, `md`,
`github` and `rdjson` formats are unversioned: a header is a JSON affair.
